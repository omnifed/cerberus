...
import { JsrLogo } from '@/components/logos/jsr'
...
export default function ApiLinks(props: ApiLinksProps) {
  return (
    ...
    >
      <Show when={Boolean(props.npm)}>
        <li>
          ...
          >
            <LogoNpm />
            npm
            <ArrowUpRight size={12} className={css({ color: 'page.text.100' })} />
          </a>
        </li>
      </Show>

      <Show when={Boolean(props.jsr)}>
        <li>
          <a
            className={apiLinkStyles}
            href={`https://jsr.io/${props.jsr}`}
            rel="noopener noreferrer"
            target="_blank"
          >
            <Square size="1.5rem">
      ...
      <Show when={Boolean(props.arkUtility)}>
        <li>
          <a
            className={apiLinkStyles}
            href={`https://ark-ui.com/react/docs/utilities/${props.arkUtility}`}
            rel="noopener noreferrer"
            target="_blank"
          >
            <svg
