...
<CodePreview preview={<NoAvatarPreview />}>
```tsx title="some-feature.tsx" {26}
import { ConfirmModal, useConfirmModal } from '@cerberus/react'
...
The ConfirmModal is an abstraction of the [Dialog primitives](./dialog) and does not provide any additional customization. To build your own ConfirmModal, use the `Dialog` components.
