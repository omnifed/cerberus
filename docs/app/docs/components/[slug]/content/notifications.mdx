...
Just update the `options.types` to change the theme of the notification. The options are `info`, `success`, `warning`, `danger`, and `loading`.
...
```tsx title="Custom Configuration" /createToaster/
'use client';
