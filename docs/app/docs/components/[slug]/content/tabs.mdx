...
Use the `orientation` prop to render tabs vertically. The `indicatorPlacement` prop can be set to "before" (it defaults to "after").

<CodePreview preview={<VerticalTabsPreview />}>
```tsx title="Vertical tabs demo"
import { Tabs } from '@cerberus/react'
import { Box } from 'styled-system/jsx'
...
| indicatorPlacement | `after`  | The position of the active tab indicator. Placement follows the document flow: `before` renders at the start edge (top or left), `after` at the end edge (bottom or right). |
