...
Once you have installed the Data Grid, we recommend adding it to the `include` array in your `panda.config.ts` file:

<CodeSnippet snippet={`export default createCerberusConfig({
    include: [
      './node_modules/@cerberus/data-grid/src/components/*.{ts,tsx}',
      // ...your other stuff,
    ],
})`} />
