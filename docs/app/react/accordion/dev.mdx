...
import { StaticPreview, IndicatorPositionPreview, SizesPreview, CustomPreview } from './components/static-preview'
...
The `AccordionItemGroup` component supports two sizes: `sm` and `lg`. This changes the size of the icon indicator displayed.

<CodePreview preview={<SizesPreview />}>
```tsx title="sizes demo" {11}
function BasicPreview() {
  return (
    <Box w="2/3">
      <Accordion defaultValue={['one']}>
