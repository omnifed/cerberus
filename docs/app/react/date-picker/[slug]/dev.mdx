...
import { StaticPreview, RangePreview } from '@/app/react/date-picker/components/static-preview'
import { RangeDefaultValuePreview } from '@/app/react/date-picker/components/range-default-value.client'
...
You can use the `formatDate` function to format a date object into a string. This is useful for displaying dates in a human-readable format.

```tsx
import { formatDate } from '@cerberus/react'
const date = new CalendarDate(2022, 2, 3);
const formattedDate = formatDate(date, 'yyyy-MM-dd'); // "2022-02-03"
```
<NoteAdmonition description={<>View the <a href="https://react-spectrum.adobe.com/internationalized/date/DateFormatter.html" target="_blank" rel="no-referrer">formatDate documentation</a> for more details on how to use it.</>} />
