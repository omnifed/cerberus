...
const fragmentShaderSource = `
  ...
  void main() {
    ...
    // --- 4. ACHERON COLORS (THEME AWARE) ---
    // Dark Mode Colors
    vec3 darkDeep = vec3(0.015, 0.03, 0.04);
    vec3 darkMid  = vec3(0.059, 0.129, 0.184); // brand.100
    vec3 darkAmbient = vec3(0.294, 0.651, 0.925) * 0.4;

    // Light Mode Colors (Daytime River)
    vec3 lightDeep = vec3(0.859, 0.929, 0.984); // brand.20
    vec3 lightMid  = vec3(0.980, 0.992, 0.996); // brand.5
    vec3 lightAmbient = vec3(0.576, 0.792, 0.957) * 0.8; // brand.40 Highlights

    // Smoothly mix between palettes based on current theme
    vec3 deepWater = mix(darkDeep, lightDeep, u_lightMode);
    vec3 midWater  = mix(darkMid, lightMid, u_lightMode);
    vec3 ambientBlue = mix(darkAmbient, lightAmbient, u_lightMode);

    // The dominant action color (#eca64b)
    vec3 actionOrange = vec3(0.925, 0.651, 0.294);

    // Build the base final color
    vec3 finalColor = mix(deepWater, midWater, baseFluid);

    // Add the ambient blue texture ripples
    finalColor += ambientBlue * blueRipples;

    // Paint the bright action lines
    // Dark mode uses additive blending (glowing light). Light mode uses alpha mixing (painting).
    vec3 darkOrangeBlend = finalColor + actionOrange * orangeLines * 0.6;
    vec3 lightOrangeBlend = mix(finalColor, actionOrange, orangeLines * 0.8);
    finalColor = mix(darkOrangeBlend, lightOrangeBlend, u_lightMode);

    // Depth Vignette
    // Dark mode darkens bottom to black. Light mode darkens bottom to a soft blue so it doesn't look muddy.
    vec3 vignetteDark = finalColor * mix(0.5, 1.0, uv.y + 0.2);
    vec3 vignetteLight = finalColor * mix(vec3(0.8, 0.85, 0.9), vec3(1.0), uv.y + 0.2);

    finalColor = mix(vignetteDark, vignetteLight, u_lightMode);
...
export default function AcheronDayBackground() {
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      // Check for 'dark' class or data-theme (covers Tailwind and next-themes)
      const isDark =
        document.documentElement.classList.contains('dark') ||
        document.documentElement.getAttribute('data-theme') === 'dark'

      // Smoothly interpolate the lightMode uniform for seamless theme transitions
