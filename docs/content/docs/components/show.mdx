...
<WarningAdmonition description="If you need to toggle an action between states based on user interaction, use the Toggle." />
...
<Example path="components/show" demo="basic" />
...
<NoteAdmonition description="The key point: By the time the <Show> component decides not to render the children, it's too late—the child component's code has already run." />
