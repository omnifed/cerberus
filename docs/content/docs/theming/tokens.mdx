...
<CodeSnippet
  snippet={`<Box color="primary" fontFamily="body">
  Hello World
</Box>`}
/>
...
<NoteAdmonition
  description="It is important to use the complete token path; for example, instead of using
...
<CodeSnippet snippet={`import { defineTokens } from '@pandacss/dev'
...
<CodeSnippet
  snippet={`type CompositeAsset = { type: 'url' | 'svg'; value: string }
